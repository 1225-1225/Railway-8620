...
![聊天主页](docs/images/chat-home.png)

**车次查询**（Agent 自动调用工具，返回结构化时刻表）：

![车次查询](docs/images/chat-train-info.png)

**交互式路线地图**（生成的地图内嵌在对话中，支持缩放/悬停）：

![路线地图](docs/images/chat-map-card.png)
...
`train_sync/` 是一个**独立子项目**，从 12306 公开接口拉取车次时刻表并转换为本项目的 JSON 格式：

```bash
cd train_sync
python run_daily.py          # 一键更新（爬取 → 转换 → 写入 ../data/）
```

- **许可隔离**：`train_sync/` 为原创实现（仅参考公开 HTTP 接口行为），与主项目通过数据文件交互，无代码链接；主项目保持 CC BY-NC 4.0
- **失败保护**：爬取被反爬/失败时不覆盖已有数据，旧数据继续可用
- **自动调度**：支持 Windows 任务计划程序 / cron / GitHub Actions（见 `train_sync/README.md`）

> ⚠️ 本仓库仅用于学习和研究目的。数据版权归各自所有者所有。请遵守 12306 网站的使用条款，控制请求频率。
